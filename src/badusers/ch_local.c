...
/*
 *  CH_BADUSERS --  Actual delivery of mail, out of transport environment
 *
...
char   *argv[];
{
    ...
    retval = ch_badusers (argc, argv);
    ll_close (logptr);              /* clean log end, if cycled  */
    exit (retval);
}
/*^L**************  (ch_) LOCAL MAIL DELIVERY  ************************ */

ch_badusers (argc, argv)                /* send to local machine               */
int     argc;
char   *argv[];
{
    ch_llinit (chanptr);
#ifdef DEBUG
    ll_log (logptr, LLOGBTR, "ch_badusers ()");
#endif

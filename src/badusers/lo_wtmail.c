...
extern char ba_info[];                /* type of delivery             */
extern char ba_sender[];      /* return address for message   */
extern char ba_adr[];         /* destination address from deliver */
extern char ba_adr_orig[];    /* destination address from deliver */
extern char ba_replyto[];     /* the reply address            */
extern char ba_size[];
extern char *ba_parm;         /* parameter portion of address */
extern struct passwd *ba_pw;  /* passwd struct for recipient  */
...
LOCVAR char *vararray[] = {
      "sender",       ba_sender,      /* 0/1 */
      "address",      ba_adr,         /* 2/3 */
      "info",         ba_info,        /* 4/5 */
      "reply-to",     ba_replyto,     /* 6/7 */
      "size",         ba_size,        /* 8/9 */
      0,              0
};
...
ba_slave ()
{
...
#ifdef DEBUG
      ll_log (logptr, LLOGBTR, "ba_slave()");
#endif
      mp = &mdlv;
      printx ("\n");
      ...
      if (rp_isgood (result = ba_dlvfile(dlvfile))
                      || result != RP_MECH)
              return (result);

      /* THIRD Attempt: system delivery file */

      if (rp_isgood (result = ba_dlvfile(sysdlvfile))
                      || result != RP_MECH)
              return (result);
...
char  *mboxname;
{
...
#ifdef        DEBUG
      ll_log (logptr, LLOGBTR, "ba_dofile(%s)", mboxname);
#endif /* DEBUG */
      ...
      retval = qu2ba_txtcpy (mbx_fd, TRUE);
      closeit:
      mbx_close (mboxname);
...
LOCFUN
qu2ba_txtcpy (ofd, tstdelim)  /* copy the text of the message       */
...
char  *prog;
{
...
#ifdef        DEBUG
      ll_log (logptr, LLOGBTR, "ba_dopipe(%s)", prog);
#endif /* DEBUG */
...
#ifdef DEBUG
              ll_log (logptr, LLOGBTR, "ba_dopipe() child (%s)", tmpbuf);
#endif
      ...
      if(rp_gval(result) != RP_OK && rp_gval(result) != RP_DONE)
              return result;
      return (ba_pwait (childid, prog)); /* .... and wait */
}

LOCFUN
ba_pwait (procid, prog)               /* wait for child to complete         */
...
char  *prog;
{
...
#ifdef DEBUG
      ll_log (logptr, LLOGBTR, "ba_pwait ()");
#endif
      ...
      {                       /* alarm is in case user's pgm hangs    */
              ...
              ll_log (logptr, LLOGBTR, "(%s) %s [%s]",
                      ba_pw->pw_name, prog, rp_valstr (status));
              if( status == 0)        /* A-OK */
              {
                      printx (", succeeded\r\n");
                      return (RP_MOK);
...
/* ^Lensure that all addresses can be passed to shell */
ba_padadr()
{
    static done_pad = 0;
    register int i;
...
/*^L*******  (mbx_)  STANDARD DELIVERY TO MAILBOX STYLE FILE  *********** */
...
/*^L****************** MAILDELIVERY HANDLING ROUTINES ***************/
...
char  *dlv_file;              /* the file to scan */
{
...
#ifdef        DEBUG
      ll_log (logptr, LLOGBTR, "ba_dlvfile(%s)", seenull(dlv_file));
#endif /* DEBUG */
      ...
      if (fstat (fileno(fp), &sb) < 0
              || (sb.st_uid != ba_pw->pw_uid && sb.st_uid != 0)
              || sb.st_mode & 022) {
      ...
      if ( mptr >= &thefile[MAXLINES] )
              ll_log (logptr, LLOGFST, "more than %d lines in %s (%s)",
                      MAXLINES, dlv_file, ba_pw->pw_name);

      maxptr = mptr;
      fclose (fp);    /* OK, file finished, take a deep breath and ... */
      return ba_dorules(thefile, maxptr);
}
...
ba_dorules(mptr, mpmax)       /* apply the maildelivwery rules */
Mdlvry        *mptr, *mpmax;
{
...
/* Run through the special case headers, break if hit, continue if miss */
                      ...
                      case    M_DEFAULT:
                              ...
                              continue;
                      case    M_ADDRESS:
                              if( strindex(mptr->m_pattern, ba_adr) >= 0)
                                      break;
                              continue;
                      case    M_TRUE: /* always true */
                              break;
                      case    M_SENDER:
                              if( strindex(mptr->m_pattern, ba_sender) >= 0)
                                      break;
                              continue;
...
{
# ifdef       DEBUG
      ll_log (logptr, LLOGBTR, "ba_doaction");
#endif /* DEBUG */
      ...
      switch( mp->m_action )
      {
              ...
              /* pipe has gone away */
              case    M_PIPE:
                      return ba_dopipe(mp->m_options);
              case    M_FILE:
                      return ba_dofile(mp->m_options);
              default:

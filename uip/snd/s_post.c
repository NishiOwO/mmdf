...
do_message ()
{
	...
	if (badflg)
	{
	    fprintf(stderr,"Make header legal and try again.\n");
	    if (rp_isbad (retval = mm_end (NOTOK)))
		    fprintf(stderr,"Can't terminate submission.\n");
	    longjmp (savej, 1);
	}
	...
	fprintf(stderr," no 'To:' or 'cc:' addressees.\n");
    ...
    if (to[0] != '\0' || bcc[0] != '\0' || cc[0] != '\0')
	printf (" Message posted.\n\n");
    else
	fprintf(stderr," *** No addresses to send to!\n\n");
}
...
char    addr[];
{
	...
	case RP_NO: 
	    fprintf(stderr,"not deliverable; unknown problem\n");
	    badflg = TRUE;
	    break;

	case RP_USER: 
	    fprintf (stderr,"not deliverable: %s\n", thereply.rp_line);
	    badflg = TRUE;
	    break;

	case RP_NDEL: 
	    fprintf(stderr,"not deliverable; permanent error.\n");
	    badflg = TRUE;
	    break;
